...
namespace Lottery.WebApp
{
    public partial class ajax : UserCenterSession
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (this._operType)
            {
                ...
                case "GetLotteryNumber":
                    ...
                    break;
                case "GetLotteryNumber1":
                    this.GetLotteryNumber1();
                    break;
        ...
        public void GetLotteryNumber1()
        {
            string detail = Public.GetOpenListJson(this.q("lcode"));

            if (detail.IndexOf("opencode") < 0)
            {
                this._response = "{\"result\":\"0\",\"returnval\": \"" + detail + "\"}";
            }
            else
            {
                this._response = "{\"result\":\"1\",\"returnval\": " + detail + "}";
            }
        }
        ...
        {
            string ltId = base.q("lid");
            string value = "0";
            string ltInfo = "{\"name\": \"名称\",\"lotteryid\": \"彩种类别\",\"ordertime\": \"倒计时\",\"closetime\": \"封单时间\",\"nestsn\": \"下期期号\",\"opennum\": \"已开期数\",\"cursn\": \"当前期号\"}";
            ltInfo = ltInfo.Replace("名称", LotteryUtils.LotteryTitle(Convert.ToInt32(ltId))).Replace("彩种类别", ltId);
            ...
            //Sys_Lottery.id=1005
            this.doh.Reset();
            this.doh.SqlCmd = "select dbo.f_GetCloseTime(" + ltId + ") as closetime";
            DataTable dataTable = this.doh.GetDataTable();
            ltInfo = ltInfo.Replace("封单时间", dataTable.Rows[0]["closetime"].ToString());
            ...
            if (ltId == "3002" || ltId == "3003")
            {
                ...
                timeSpan = Convert.ToDateTime(value2) - Convert.ToDateTime(time);
            }
            #endregion
            else
            {
                ...
                //单个开奖记录
                //Sn期号
                string condtion = string.Format("Time>'{0}' and LotteryId={1} ", time, ltId);
                DataRow[] array = UserCenterSession.LotteryTime.Select(condtion, "Time asc");

                if (array.Length == 0)
                {
                    condtion = string.Format("Time<='{0}' and LotteryId={1} ", time, ltId);
                    array = UserCenterSession.LotteryTime.Select(string.Concat(condtion, "Time asc"));
                ...
                else
                {
                    ...
                    {
                        if (ltId == "1014" || ltId == "1016")
                        {
                            text7 = dateTime.AddDays(1.0).ToString("yyyyMMdd") + "-" + array[0]["Sn"].ToString();
                        }
                    }
                }
                if (Convert.ToDateTime(array[0]["Time"].ToString()) < Convert.ToDateTime(time))
                {
                    d = Convert.ToDateTime(dateTime.AddDays(1.0).ToString("yyyy-MM-dd") + " " + array[0]["Time"].ToString());
                }

                timeSpan = d - Convert.ToDateTime(time);
                condtion = string.Format("Time<'{0}' and LotteryId={1} ", time, ltId);
                DataRow[] array2 = UserCenterSession.LotteryTime.Select(condtion, "Time desc");

                if (array2.Length == 0)
                {
                    condtion = string.Format("LotteryId={0} ", ltId);
                    array2 = UserCenterSession.LotteryTime.Select(condtion, "Time desc");
                    text6 = dateTime.AddDays(-1.0).ToString("yyyyMMdd") + "-" + array2[0]["Sn"].ToString();
                    value = array2[0]["Sn"].ToString(); //期号
                }
                else
                {
                ...
                {
                    text6 = string.Concat(new LotteryTimeDAL().GetTsIssueNum(ltId) + Convert.ToInt32(array2[0]["Sn"].ToString()));
                    value = array2[0]["Sn"].ToString();
            ...
            ltInfo = ltInfo.Replace("下期期号", text7).Replace("当前期号", text6).Replace("倒计时", newValue).Replace("已开期数", Convert.ToInt32(value).ToString());
