  ...
  public class UserBetDAL : ComData
  {
        ...
        try
        {
          ...
          foreach (DataRow row in (InternalDataCollectionBase) dataTable.Rows)
          {
            ...
            if (flag)
            {
              ...
              if (new UserTotalTran().MoneyOpers(ssId, row["UserId"].ToString(), Money, int32_1, Convert.ToInt32(row[nameof (PlayId)].ToString()), Convert.ToInt32(str), 6, 99, string.Empty, string.Empty, "会员撤单", row["STime"].ToString()) > 0)
              {
    ...
    public void GetBetInfoJSON(string Id, string UserId, ref string _jsonstr)
    {
      using (DbOperHandler dbOperHandler = new ComData().Doh())
      {
        ...
        DataTable dataTable1 = dbOperHandler.GetDataTable();
        string betDetail = BetDetailDAL.GetBetDetail(Convert.ToDateTime(dataTable1.Rows[0]["STime2"]).ToString("yyyyMMdd"), dataTable1.Rows[0][nameof (UserId)].ToString(), Id);
