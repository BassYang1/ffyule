    ...
    public string UpdateParentId(string Id, string UserName, string Point, string UserGroup, string Code)
    {
      using (SqlConnection sqlConnection = new SqlConnection(ComData.connectionString))
      {
        ...
        try
        {
          ...
          if (Convert.ToDecimal(dataTable1.Rows[0][nameof (Point)]) / new Decimal(10) < Convert.ToDecimal(Point))
            return "返点高于切入账号的返点！不能切线";
          if (Convert.ToDecimal(dataTable1.Rows[0][nameof (UserGroup)]) < Convert.ToDecimal(UserGroup))
          ...
          if (dataTable2.Rows.Count > 0)
          {
            ...
            {
              sqlCommand.CommandText = "update N_User set UserCode=Replace(UserCode,'" + Code + "','" + str + "') where Id=" + dataTable2.Rows[index][nameof (Id)].ToString();
              sqlCommand.ExecuteNonQuery();
            }
          }
          sqlCommand.CommandText = "update N_User set ParentId=" + dataTable1.Rows[0][nameof (Id)] + ",UserCode='" + dataTable1.Rows[0]["UserCode"] + Strings.PadLeft(Id) + "' where Id=" + Id;
