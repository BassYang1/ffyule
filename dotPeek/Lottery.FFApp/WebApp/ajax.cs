  ...
  public class ajax : UserCenterSession
  {
    ...
    private void ajaxLotteryTime23()
    {
      ...
      if (dataTable2.Rows.Count > 0)
      {
        ...
        string newValue4 = string.Concat((object) Decimal.op_Increment(Convert.ToDecimal(dataTable2.Rows[0]["title"].ToString())));
    ...
    private void ajaxLotteryTime()
    {
      ...
      string str2 = "0";
      string str3 = "{\"name\": \"名称\",\"lotteryid\": \"彩种类别\",\"ordertime\": \"倒计时\",\"closetime\": \"封单时间\",\"nestsn\": \"下期期号\",\"opennum\": \"已开期数\",\"cursn\": \"当前期号\"}".Replace("名称", LotteryUtils.LotteryTitle(Convert.ToInt32(str1))).Replace("彩种类别", str1);
      DateTime dateTime1 = DateTime.Now;
      DateTime dateTime2 = this.GetDateTime();
      string str4 = dateTime2.ToString("yyyyMMdd");
      string str5 = dateTime2.ToString("HH:mm:ss");
      ...
      DataTable dataTable = this.doh.GetDataTable();
      string str6 = str3.Replace("封单时间", dataTable.Rows[0]["closetime"].ToString());
      int num;
      string newValue1;
      string newValue2;
      TimeSpan timeSpan;
      if (str1 == "3002" || str1 == "3003")
      {
        ...
        int Num = Convert.ToInt32(this.doh.GetDataTable().Rows[0]["d"]) - 7 + 1;
        string str7 = dateTime2.AddDays(-1.0).ToString("yyyy-MM-dd") + " 20:30:00";
        string str8 = dateTime2.ToString("yyyy-MM-dd") + " 20:30:00";
        if (dateTime2 > Convert.ToDateTime(dateTime2.ToString(" 20:30:00")))
          str8 = dateTime2.AddDays(1.0).ToString("yyyy-MM-dd") + " 20:30:00";
        else
          --Num;
        num = dateTime2.Year;
      ...
      else
      {
        if (UserCenterSession.LotteryTime == null)
          UserCenterSession.LotteryTime = new LotteryTimeDAL().GetTable();
        DataRow[] dataRowArray1 = UserCenterSession.LotteryTime.Select("Time >'" + str5 + "' and LotteryId=" + str1, "Time asc");
        if (dataRowArray1.Length == 0)
        {
          dataRowArray1 = UserCenterSession.LotteryTime.Select("Time <='" + str5 + "' and LotteryId=" + str1, "Time asc");
          newValue2 = dateTime2.AddDays(1.0).ToString("yyyyMMdd") + "-" + dataRowArray1[0]["Sn"].ToString();
        }
        else
        {
          ...
          if (dateTime2 > Convert.ToDateTime(dateTime2.ToString("yyyy-MM-dd") + " 00:00:00") && dateTime2 < Convert.ToDateTime(dateTime2.ToString("yyyy-MM-dd") + " 10:00:01") && str1 == "1003")
            newValue2 = dateTime2.AddDays(-1.0).ToString("yyyyMMdd") + "-" + dataRowArray1[0]["Sn"].ToString();
          if (dateTime2 > Convert.ToDateTime(dateTime2.ToString("yyyy-MM-dd") + " 23:00:00") && dateTime2 < Convert.ToDateTime(dateTime2.ToString("yyyy-MM-dd") + " 23:59:59") && (str1 == "1014" || str1 == "1016"))
            newValue2 = dateTime2.AddDays(1.0).ToString("yyyyMMdd") + "-" + dataRowArray1[0]["Sn"].ToString();
        }
        if (Convert.ToDateTime(dataRowArray1[0]["Time"].ToString()) < Convert.ToDateTime(str5))
          dateTime1 = Convert.ToDateTime(dateTime2.AddDays(1.0).ToString("yyyy-MM-dd") + " " + dataRowArray1[0]["Time"].ToString());
        timeSpan = dateTime1 - Convert.ToDateTime(str5);
        DataRow[] dataRowArray2 = UserCenterSession.LotteryTime.Select("Time <'" + str5 + "' and LotteryId=" + str1, "Time desc");
        if (dataRowArray2.Length == 0)
        {
      ...
      string str9 = str6.Replace("下期期号", newValue2).Replace("当前期号", newValue1).Replace("倒计时", newValue3);
