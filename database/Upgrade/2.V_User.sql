...
,Email
--是否签订工资契约
,ISNULL((SELECT DISTINCT IsUsed FROM N_UserContract WHERE UserId = a.Id AND Type = 2), -1) as ContractGZ
--是否签订分红契约
,ISNULL((SELECT DISTINCT IsUsed FROM N_UserContract WHERE UserId = a.Id AND Type = 1), -1) as ContractFH
FROM [N_User] a
