                    ...
                    if (d.table.length > 0) {
                       ...
                       html += '</colgroup>';
                        ...
                        for (var i = 0; i < d.table.length; i++) {
                            var t = d.table[i];
                                    html += '<tr>';
                                    html += '<td>' + (i + 1) + '</td><td><label class="lab">半月平均日量</label><label class="lab">' + t.minmoney + '万</label></td><td><label class="lab">' + t.money + '%</label></td><td>&nbsp;</td>';
									html += '</tr>';
                                }
                                html += '</tbody>';
                                html += '</table>';
                        if (d.table[0].groupid == 4) {
                            $i("info").innerHTML = "您是招商级别，直接用平台分红标准";
                            $("#add").html(html);
                        }
                        else if (d.table[0].groupid == 3) {
                            $i("info").innerHTML = "您是特权直属级别，直接用平台分红标准";
                            $("#add").html(html);
                        }
                        else {
