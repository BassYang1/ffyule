...
namespace Lottery.WebApp
{
    public partial class ajaxBetting : UserCenterSession
    {
        ...
        /// 下注
        /// <remarks>
        /// [{"LotteryId":"5005","PlayId":"99508","Price":2,"PriceName":"元","times":1,"Num":8,"singelBouns":"207.36","Point":0,"balls":"11_12_13_14_15_16_17_18","strPos":"","PlayName":"和值","alltotal":16},{"LotteryId":"5005","PlayId":"99508","Price":2,"PriceName":"元","times":1,"Num":8,"singelBouns":"207.36","Point":0,"balls":"4_6_8_10_12_14_16_18","strPos":"","PlayName":"和值","alltotal":16}]
        /// </remarks>
        /// </summary>
        private void ajaxBetting2()
        {
            ...
            else
            {
                ...
                Decimal betSumTotal = new Decimal(); //下注金额
                try
                {
                    string[] issueTimeAndSn = betDal.GetIssueTimeAndSN(lotteryId); //获取本期开奖时间和期号
                    string curSn = issueTimeAndSn[0]; //本期期号
                    DateTime curOpenTime = Convert.ToDateTime(issueTimeAndSn[1]); //本期开奖时间
                    DateTime serverTime = FlexDAL.PublicDAL.GetServerTime();
                    for (int index = 0; index < requestDataJsonList.Count; ++index)
                    {
                        ...
                        betSumTotal += requestDataJson2.price * (Decimal)requestDataJson2.Num * requestDataJson2.times;
                    }

                    //检查投注是否有效
                    string errMsg = betDal.CheckBet(userId, lotteryId, Convert.ToDecimal(betSumTotal), curOpenTime);
                    if (!string.IsNullOrEmpty(errMsg))
                    {
                        this._response = this.JsonResult(0, errMsg);
                    }
                    else
                    {
                        int betId = 0;
                        int betCount = requestDataJsonList.Count; //下注数量

                        //循环处理单个下注数据
                        for (int index = 0; index < betCount; ++index)
                        {
                            ajaxBetting.RequestDataJSON requestDataJson2 = requestDataJsonList[index];
                            decimal price = Convert.ToDecimal(requestDataJson2.price); //单注金额
                            int betNum = Convert.ToInt32(requestDataJson2.Num); //下注个数，选中号码个数，一个号码可能算一注
                            decimal betTimes = Convert.ToDecimal(requestDataJson2.times); //下注倍数
                            string playId = requestDataJson2.playId.ToString(); //小类Id
                            string balls = requestDataJson2.balls; //选中号码
                            string strPos = requestDataJson2.strPos; //选中位置
                            decimal point = Convert.ToDecimal(requestDataJson2.Point); //调节的返点, 0到用户返点范围内某个值

                            this.doh.Reset();
                            this.doh.ConditionExpress = "Id=@Id";
                            this.doh.AddConditionParameter("@Id", (object)requestDataJson2.playId.ToString()); //小类Id
                            string smType = string.Concat(this.doh.GetField("Sys_PlaySmallType", "Title2")); //玩法小类数量

                            Decimal singelBouns = new Decimal();
                            if (lotteryId != 5001)
                            {
                                if (price < Decimal.Zero || betNum < 0 || betTimes < Decimal.One)
                                {
                                    this._response = this.JsonResult(0, "投注错误！请重新投注！");
                                    return;
                                }

                                //计算singelBouns和验证投注个数是否正确
                                errMsg = Calculate.BetNumerice(userId, lotteryId, balls, playId, strPos, betNum, point, ref singelBouns);

                                if (!string.IsNullOrEmpty(errMsg))
                                {
                                    this._response = errMsg.Replace("[", "").Replace("]", "");
                                    return;
                                }
                                if (singelBouns <= Decimal.Zero)
                                {
                                ...
                                //验证总金额是否超过最大允许值
                                if (price * betNum * betTimes >= new Decimal(1000000))
                                {
                            ...
                            model.PlayCode = smType;
                            model.IssueNum = curSn;
                            ...
                            model.STime = curOpenTime;
                            model.STime2 = serverTime;
                            model.IsDelay = 0;
                            model.Times = Convert.ToDecimal(requestDataJson2.times);
                            model.ZHID = 0;

                            betId = !model.Pos.Equals("") ?
                                betDal.InsertBetPos(model, "Web端") : //任选
                        ...
                        if (betId > 0)
                            this._response = this.JsonResult(1, "第" + curSn + "期投注成功，请期待开奖！");
                        else
                            this._response = this.JsonResult(0, "对不起,投注失败！");
