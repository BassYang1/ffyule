...
namespace Lottery.DAL
{
    public class UserRegLinkDAL : ComData
    {
        ...
        public void SaveUserRegLink(string UserId, Decimal Point, string YxTime, string Times, string Url, string userGroup)
        {
            using (DbOperHandler dbOperHandler = new ComData().Doh())
            {
                dbOperHandler.Reset();
                ...
                if (dbOperHandler.Exist("N_UserRegLink"))
                {
                    ...
                    dbOperHandler.AddFieldItem("Url", (object)Url);
                    dbOperHandler.AddFieldItem("UserGroup", (object)userGroup);
                    dbOperHandler.Update("N_UserRegLink");
                }
                else
                {
