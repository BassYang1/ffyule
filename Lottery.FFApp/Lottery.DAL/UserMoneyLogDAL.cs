...
namespace Lottery.DAL
{
    public class UserMoneyLogDAL : ComData
    {
        ...
        public void AgencyPoint(SqlCommand cmd, int UserId, int LotteryId, int PlayId, int BetId, Decimal BetMoney)
        {
            ...
            sqlDataAdapter.SelectCommand.CommandText = "select ParentId,Point,Money from N_User with(nolock) where Id=" + parentId.ToString();
            sqlDataAdapter.Fill(dataTable);
            if (dataTable.Rows.Count <= 0)
                return;

            DataRow row = dataTable.Rows[0];
            int parentPoint = Convert.ToInt32(row["Point"]); //父级返点
            cmd.CommandText = "select top 1 MoneyAfter From N_UserMoneyLog with(nolock) Where UserId=" + parentId.ToString() + " order by Id desc";
            ...
            if (parentPoint < int32_3)
                return;
            Decimal num = BetMoney * Convert.ToDecimal(parentPoint - int32_3) / new Decimal(1000);
            new UserMoneyLogDAL().Save(cmd, parentId, LotteryId, PlayId, BetId, num, 7, 99, "下级返点");
            new UserMoneyStatDAL().Save(cmd, parentId, "Point", num);
            this.AgencyPoint(cmd, parentId, LotteryId, PlayId, BetId, BetMoney);
            dataTable.Dispose();
        }
    }
}
