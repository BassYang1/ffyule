    ...
    public class Calculate
    {
        ...
        public static string BetNumerice(int userId, int lotteryId, string balls, string playId, string pos, int betnum, Decimal Point, ref Decimal singelBouns)
        {
            ...
            using (DbOperHandler dbOperHandler = new ComData().Doh())
            {
                ...
                Decimal userPoint = string.IsNullOrEmpty(string.Concat(field)) ? new Decimal(0) : Convert.ToDecimal(string.Concat(field));
                dbOperHandler.Reset();
                ...
                //最大赔率
                Decimal maxBonus = Convert.ToDecimal(dataTable.Rows[0]["MaxBonus"].ToString());

                //最小赔率
                Decimal minBonus = Convert.ToDecimal(dataTable.Rows[0]["MinBonus"].ToString());
                //Convert.ToDecimal(dataTable.Rows[0]["MinBonus2"].ToString());

                //位置赔率
                Decimal posBonus = Convert.ToDecimal(dataTable.Rows[0]["PosBonus"].ToString());
                ...
                if (posBonus != new Decimal(0))
                {
                    if (lotteryId != 5001)
                    {
                        if (Point > userPoint || Point < new Decimal(0))
                            return Calculate.JsonResult(0, "投注失败,返点错误，请重新投注！");

                        //最小赔率 + (用户返点 - 奖金返点 * 10) * 2 * posBonus
                        //奖金返点在页面可以调节
                        //singelBouns = minBonus + (userPoint - Point * 10) * 2 * posBonus;

                        //固定赔率(最大赔率和最小赔率相同)
                        singelBouns = minBonus;
                    }
                }
            }
            ...
            { 
                ...
                case "P11_RXTD7":
                case "P11_RXTD8":
                    num5 = Calculate.Red11X5TD(balls, str1.Replace("P11_RXTD", ""));
                    break;
                case "K_3HZ": //快三
                case "K_3STDX":
                case "K_32TDX":
