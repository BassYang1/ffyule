...
namespace Lottery.DAL
{
        ...
        private byte[] Keys = new byte[8]
    {
      ...
      (byte) 239
        ...
        protected void getSysLotteryDropDownList(ref DropDownList ddlClassId)
        {
            if (this.Page.IsPostBack)
                return;
            this.doh.Reset();
            this.doh.SqlCmd = "SELECT [Id],[Title] FROM Sys_Lottery WHERE ISNULL(IsSys, 0)=1 ORDER BY Id asc";
            DataTable dataTable = this.doh.GetDataTable();
            if (dataTable.Rows.Count == 0)
            {
                dataTable.Clear();
