...
namespace Lottery.DAL.Flex
{
    public class UserBetDAL : ComData
    {
        ...
        /// <param name="lotteryId"></param>
        /// <param name="betSumTotal">本期投注金额</param>
        /// <param name="curOpenTime">本期开奖时间</param>
        /// <returns></returns>
        public string CheckBet(int UserId, int lotteryId, Decimal betSumTotal, DateTime curOpenTime)
        {
            if (UserId == 0)
            ...
            using (DbOperHandler dbOperHandler = new ComData().Doh())
            {
                ...
                int isOpen = Convert.ToInt32(dataTable1.Rows[0]["IsOpen"]); //是否启用彩种
                int closeTime = Convert.ToInt32(dataTable1.Rows[0]["CloseTime"]); //封单时间

                if (isOpen != 0)
                    return "暂停投注，请与客服联系。";

                dbOperHandler.Reset();
                dbOperHandler.SqlCmd = "select DATEDIFF(S,GETDATE(),'" + curOpenTime.ToString("yyyy-MM-dd HH:mm:ss") + "') as time,GETDATE() as now";
                DataTable dataTable2 = dbOperHandler.GetDataTable();

                if (Convert.ToDateTime(dataTable2.Rows[0]["now"]) > curOpenTime || Convert.ToInt32(dataTable2.Rows[0]["time"]) <= closeTime)
                    return "本期已封单,不能投注!";
                ...
                int isBet = Convert.ToInt32(dataTable3.Rows[0]["IsBet"]); //是否允许投注
                int isEnable = Convert.ToInt32(dataTable3.Rows[0]["IsEnable"]); //是否禁用账号

                //用户返点
                Decimal userPoint = string.IsNullOrEmpty(string.Concat(dataTable3.Rows[0]["Point"])) ? new Decimal(0) : Convert.ToDecimal(dataTable3.Rows[0]["Point"]);
                //用户账户金额
                Decimal userMoney = Convert.ToDecimal(dataTable3.Rows[0]["Money"]);
                ...
                //大小于系统允许投注最大点数
                if (userPoint >= this.site.MaxLevel * new Decimal(10))
                    return "当前帐号无法投注，请联系客服处理!";

                if (userMoney < betSumTotal)
                    return "投注失败,您的帐号余额不足!";
