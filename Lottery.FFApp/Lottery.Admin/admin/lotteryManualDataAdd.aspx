<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="lotteryManualDataAdd.aspx.cs" Inherits="Lottery.AdminFile.Admin.lotteryManualDataAdd"  EnableEventValidation="false"  %>
...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            GetLotteyIssueNum();
            $(".sysLottery").change(function () {
                GetLotteyIssueNum();
            });

            $("#ddlIssueNum").change(function () {
                GetLotteyData();
            });

        });

        function GetLotteyIssueNum() {
            var lotId = $(".sysLottery").val();
            ...
            $.ajax({
                ...
                async: false,
                data: "",
                url: "/admin/ajaxLotterydata.aspx?oper=ajaxGetIssueNum&lotId=" + lotId + "&clienttime=" + Math.random(),
                error: function (XmlHttpRequest, textStatus, errorThrown) { },
                success: function (d) {
                    var options = "<option value='0'>请选开奖期数</option>";
                    for (var i = 0; i < d.table.length; i++) {
                        options += "<option value='" + d.table[i].sn + "'>" + d.table[i].sn + "</option>";
                    }

                    $("#ddlIssueNum").html(options);
                }
            });
        }

        function GetLotteyData() {
            var lotId = $(".sysLottery").val();
            var sn = $("#ddlIssueNum").val();
            ...
            $.ajax({
                ...
                async: false,
                data: "",
                url: "/admin/ajaxLotterydata.aspx?oper=ajaxGetLotteryData&lotId=" + lotId + "&sn=" + sn + "&clienttime=" + Math.random(),
                error: function (XmlHttpRequest, textStatus, errorThrown) { },
                success: function (d) {
                    if (d.result == "1") {
                        $("#txtTitle").val(d.no);
                        $("#txtNumber").val(d.number);
                        $("#txtOpenTime").val(d.time);
                        $("#txtNumberAll").val(d.numberall);
...
<body>
    ...
    <table class="formtable">
        <tr>
            ...
            <td>
               <asp:DropDownList ID="ddlType" CssClass="sysLottery" runat="server" Height="35px" Width="200"></asp:DropDownList>
            </td>
        </tr>
        <tr>
            <th>
            ...
            <td>
               <asp:DropDownList ID="ddlIssueNum" runat="server" Height="35px" Width="200"></asp:DropDownList>
            </td>
        </tr>
        <tr>
            <th>
            ...
            <td>
                <asp:HiddenField ID="txtNumberAll" runat="server" />
