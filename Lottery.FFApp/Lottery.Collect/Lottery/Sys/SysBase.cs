...
namespace Lottery.Collect.Sys
{
    ...
    public abstract class SysBase
    {
        ...
        /// 初使化彩种期号
        /// </summary>
        public void GetLotteryData(string ltId, string code, string sn)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(Const.ConnectionString))
                {
                    using (DbOperHandler doh = new SqlDbOperHandler(conn))
                    {
                        ...
                        string curExpect = "";
                        DataRow[] dataRowArray2 = UserCenterSession.LotteryTime.Select("LotteryId=" + ltId + " and Sn='" + sn + "'");

                        if (dataRowArray2.Length > 0)
                        {
                            curExpect = curDateTime.ToString("yyyyMMdd") + "-" + sn;
                                ...
                                && ltId == "1003")
                            {
                                ...
                                curExpect = curDateTime.AddDays(-1.0).ToString("yyyyMMdd") + "-" + sn;
                            }
                        ...
                        {
                            curExpect = string.Concat((object)(new LotteryTimeDAL().GetTsIssueNum(ltId) + Convert.ToInt32(sn)));
                        }

                        //新加坡2分彩
                        if (ltId == "1012")
                        {
                            curExpect = string.Concat((object)(new LotteryTimeDAL().GetTsIssueNum("1012") + Convert.ToInt32(sn)));
                        }

                        //台湾5分彩
                        if (ltId == "1013")
                        {
                            curExpect = string.Concat((object)(new LotteryTimeDAL().GetTsIssueNum("1013") + Convert.ToInt32(sn)));
                        }
                        ...
                        if (ltId == "4001")
                        {
                            curExpect = string.Concat((object)(new LotteryTimeDAL().GetTsIssueNum("4001") + Convert.ToInt32(sn)));
                        }

                        this.Expect = Int32.Parse(sn); //开奖期数
        ...
        {
            //预设开奖
            DataTable dataTable1 = LotteryDAL.GetPreDataTable(this.Id.ToString(), this.ExpectNo);

            if (dataTable1 != null && dataTable1.Rows.Count > 0)
            {
                this.Number = dataTable1.Rows[0]["Number"].ToString();
                this.NumberAll = dataTable1.Rows[0]["NumberAll"].ToString();
                return;
            }
        ...
        public void UpdateLottery()
        {
            try
            {
                ...
                {
                    Public.SaveLotteryData2File(this.Id, false);
