    ...
    public static class ZdbHelper
    {
        ...
        public static string RSASign(string signStr, string privateKey)
        {
            try
            {
                ...
                byte[] signBytes = rsa.SignData(UTF8Encoding.UTF8.GetBytes(signStr), ZDB_SIGN);
        ...
        public static bool ValidateRsaSign(string plainText, string publicKey, string signedData)
        {
            try
            {
                ...
                return rsa.VerifyData(UTF8Encoding.UTF8.GetBytes(plainText), ZDB_SIGN, Convert.FromBase64String(signedData));
            }
            catch (Exception e)
            {
                throw e;
