<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="chargeWfp.aspx.cs" Inherits="Lottery.Web.money.chargeWfp" %>
    ...
    <script type="text/javascript">
        ...
        function ajaxList() {
            ...
            $.ajax({
                ...
                url: "/ajax/ajaxBank.aspx?oper=ajaxGetChargeSetList&code=weifupay",
                    ...
                    $.ajax({
                        ...
                        success: function (d) {
                            if (d.result == "1") {
                                ...
                                if ($(".bank:checked").val() == "aliapi_h5api") {
                                    $(".payType").html("支付宝");
                                }
                                else if ($(".bank:checked").val() == "weixin_h5api") {
                                    $(".payType").html("微信");
                                }
                            }
                            else {
        ...
        function submitPay() {
            ...
            if (bank && userId && userId != "0") {
                url = getHost() + "/pay/weifupay/pay.aspx?bank=" + bank + "&setCode=weifupay&amount=" + uMoney + "&userId=" + userId + "&orderId=" + orderId + "&clienttime=" + Math.random();
                ...
                <div class="form-item">
                    ...
                    <div class="item-content">
                        <div class="item-content-text">
                            <%--<label><input type="radio" value="alipay_h5api" title="支付宝移动支付" class="radio bank" name="bank" checked/>支付宝</label>
                            <label><input type="radio" value="weixin_h5api" title="微信移动支付" class="radio bank" name="bank" checked />微信</label>--%>
                            <label><input type="radio" value="qq_h5api" title="QQ移动支付" class="radio bank" name="bank" checked/>QQ</label>
                            <%--<label><input type="radio" value="unionpay_h5api" title="银联移动支付" class="radio bank" name="bank" checked />银联</label>--%>
