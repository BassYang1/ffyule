  ...
  public partial class ajaxBetting : UserCenterSession
  {
    ...
    private void ajaxBetting2()
    {
      ...
      else
      {
        ...
        try
        {
          string[] issueTimeAndSn = new Lottery.DAL.Flex.UserBetDAL().GetIssueTimeAndSN(lotteryId1);
          string str1 = issueTimeAndSn[0];
          DateTime dateTime = Convert.ToDateTime(issueTimeAndSn[1]);
          DateTime serverTime = PublicDAL.GetServerTime();
          ...
          string str2 = new Lottery.DAL.Flex.UserBetDAL().CheckBet(int32_1, lotteryId1, Convert.ToDecimal(num1), dateTime);
          if (!string.IsNullOrEmpty(str2))
          {
            this._response = this.JsonResult(0, str2);
          }
          else
          {
            ...
            for (int index = 0; index < requestDataJsonList.Count; ++index)
            {
              ...
              model.ZHID = 0;
              num2 = !model.Pos.Equals("") ? new Lottery.DAL.Flex.UserBetDAL().InsertBetPos(model, "手机端") : (!model.PlayCode.Equals("P_5ZH") && !model.PlayCode.Equals("P_4ZH_L") && (!model.PlayCode.Equals("P_4ZH_R") && !model.PlayCode.Equals("P_3ZH_L")) && !model.PlayCode.Equals("P_3ZH_C") && !model.PlayCode.Equals("P_3ZH_R") ? new Lottery.DAL.Flex.UserBetDAL().InsertBet(model, "手机端") : new Lottery.DAL.Flex.UserBetDAL().InsertBetZH(model, "手机端"));
            }
            if (num2 > 0)
