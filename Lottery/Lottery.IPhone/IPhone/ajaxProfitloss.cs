...
namespace Lottery.IPhone
{
	public class ajaxProfitloss : UserCenterSession
	{
		...
		private void ajaxGetProListId()
		{
			...
			if (Convert.ToDateTime(text3) > Convert.ToDateTime(text))
			{
				text3 = text;
			}
			this.doh.Reset();
			...
			if (text6.Length > 1)
			{
				...
				dt = this.GetUserMoneyStat(text3, text, text6);
				this.doh.Reset();
				...
				if (text7.Length > 1)
				{
					...
					dtsum = this.GetUserMoneyStat(text3, text, text7);
				}
		...
		private void ajaxGetProList()
		{
			...
			if (string.IsNullOrEmpty(text) || text == "0")
			{
				...
				string text4 = base.q("d1");
				string text5 = base.q("d2");
