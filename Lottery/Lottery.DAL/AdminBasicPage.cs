	...
	public class AdminBasicPage : PageUI
	{
		...
		public override void ConnectDb()
		{
			if (this.doh == null)
			{
				try
				{
					this.doh = new SqlDbOperHandler(new SqlConnection(Const.ConnectionString));
				}
				catch (Exception ex)
				{
					throw ex;
		...
		protected void FinalMessage(string pageMsg, string go2Url, int BackStep, int Seconds)
		{
			...
			if (BackStep > 0)
			{
				stringBuilder.Append("        <a href=\"javascript:history.go(" + -BackStep + ")\">如果您的浏览器没有自动跳转，请点击这里</a>\r\n");
			}
			else
			{
		...
		protected void SetupSystemDate()
		{
			...
			HttpContext.Current.Application["AdminLottery"] = this.site;
