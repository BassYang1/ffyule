	...
	public class ContractDAL : ComData
	{
		public void IsContract(string UserId, string strWhere, ref string _jsonstr)
		{
			using (DbOperHandler dbOperHandler = new ComData().Doh())
			{
				...
				string text2 = "0";
				dbOperHandler.Reset();
				dbOperHandler.SqlCmd = string.Format("SELECT top 1 Id from N_UserContract where type=1 and UserId=" + UserId + strWhere, new object[0]);
		...
		public void GetContractInfo(string UserId, ref string _jsonstr)
		{
			using (DbOperHandler dbOperHandler = new ComData().Doh())
			{
				...
				if (dataTable.Rows.Count > 0)
				{
					...
					{
						dbOperHandler.Reset();
						dbOperHandler.SqlCmd = string.Format("SELECT 4 as [GroupId],[MinMoney],[Group3] as money,[IsUsed],[soft] FROM [Act_Day15FHSet] where GroupId=4 and IsUsed=0", new object[0]);
						dataTable = dbOperHandler.GetDataTable();
					...
					{
						dbOperHandler.Reset();
						dbOperHandler.SqlCmd = string.Format("SELECT 3 as [GroupId],[MinMoney],[Group3] as money,[IsUsed],[soft] FROM [Act_Day15FHSet] where GroupId=3 and IsUsed=0", new object[0]);
						dataTable = dbOperHandler.GetDataTable();
						_jsonstr = "{\"result\" :\"1\",\"returnval\" :\"操作成功\"," + dtHelp.DT2JSONNOHTML(dataTable, 0, "recordcount", "table", true) + "}";
					}
					...
					{
						dbOperHandler.Reset();
						dbOperHandler.SqlCmd = string.Format("SELECT 0 as groupId,[Type],[ParentId],[UserId],[IsUsed],[STime],b.*\r\n                                            FROM [N_UserContract] a left join [N_UserContractDetail] b on a.Id=b.UcId where Type=1 and UserId=" + UserId, new object[0]);
						dataTable = dbOperHandler.GetDataTable();
						_jsonstr = "{\"result\" :\"1\",\"returnval\" :\"操作成功\"," + dtHelp.DT2JSONNOHTML(dataTable, 0, "recordcount", "table", true) + "}";
					}
				}
		...
		public int SaveContract(UserContract list)
		{
			...
			if (list.UserContractDetails.Count > 0)
			{
				using (SqlConnection sqlConnection = new SqlConnection(ComData.connectionString))
				{
					...
					try
					{
						sqlCommand.CommandText = string.Format("delete from [N_UserContract] where Type=1 and UserId={0}", list.UserId);
						sqlCommand.ExecuteScalar();
						sqlCommand.CommandText = string.Format("INSERT INTO [N_UserContract]([Type],[ParentId],[UserId],[IsUsed],[STime]) VALUES (1,{0},{1},0,getdate())", list.ParentId, list.UserId);
