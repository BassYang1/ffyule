...
namespace Lottery.DAL
{
	public static class GYoule11x5Check
	{
		public static void RunOfIssueNum(int LotteryId, string IssueNum)
		{
			...
			doWord.BeginInvoke(LotteryId, IssueNum, new AsyncCallback(GYoule11x5Check.CallBack), doWord);
		}

		public static void CallBack(IAsyncResult r)
		{
			GYoule11x5Check.DoWord arg_0B_0 = (GYoule11x5Check.DoWord)r.AsyncState;
		}
				...
				if (dataTable.Rows.Count > 0)
				{
					...
					if (curRealGet < Convert.ToDecimal(lotteryCheck.Rows[0]["CheckPer"]))
					{
						...
						do
						{
							decimal d = 0m;
							decimal num3 = 0m;
							string text = NumberCode.CreateCode11X5(5);
						...
						GYoule11x5Check.SetOpenListJson(LotteryId, IssueNum, list2[0].tKey, DateTime.Now.ToString(), string.Concat(list2[0].tValue));
					}
					else
					{
						string opencode = NumberCode.CreateCode11X5(5);
						GYoule11x5Check.SetOpenListJson(LotteryId, IssueNum, opencode, DateTime.Now.ToString(), "0");
					}
				}
				else
				{
					string opencode2 = NumberCode.CreateCode11X5(5);
					GYoule11x5Check.SetOpenListJson(LotteryId, IssueNum, opencode2, DateTime.Now.ToString(), "0");
				}
			}
			catch (Exception ex)
			{
